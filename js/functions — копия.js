...
map.on('draw:edited', function (e) {
	allowMark = true;
	for (var i in e.layers._layers) {
		console.log(e.layers._layers[i].defaultOptions.markId);
...
function dodo(data) {
	...
	for (var i in ss.markUser) {
		...
		if (markOwn == userIp) {
			...
			if (ss.read == 0 || ss.own == userIp) {
			// ОБНОВЛЕНИИ ПОЗИЦИИ ЗАГРУЖЕННОГО МАРКЕРА
			...
			// ПКМ - МЕНЮ ЗАГРУЖЕННОГО МАРКЕРА
			marU.on("contextmenu", function(e) {
				var theMark = this;
				var theMarId = theMark.options.markId;
				var htmMark = `
				<div class="modalNamed">
				<span class="namedMark">Наименовать маркер:</span>
				<input id="nMark" type="text" placeholder="Название" maxlength="40">
